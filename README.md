...
**86 rows have no coordinates** (54 have no location text at all). They are
ingested with `is_mappable=False`: searchable and visible in film detail, absent
from the map. Dropping them would silently lose 4% of the dataset.
...
- **Data freshness depends on the sync run.** There is no scheduler in the
  repository; the crontab line is above.
